...
static void ori_dscal_kernel(long n, double *alpha, double *x)
{
    long i = 0;
    long err_num = 0;
...
void ori_dscal_compute(long int n, double da, double *x, long int inc_x)
{
    long int i = 0;
    if (inc_x == 1)
    {
...
void ftblas_dscal_ori(const long int n, const double alpha, const double *x, const long int inc_x)
{
  ...
  {
    ori_dscal_compute(n,alpha,(double *)x,inc_x);
    return;
  }
  int tid;
...
#pragma omp parallel for schedule(static)
  for (tid = 0; tid < TOTAL_THREADS; tid++)
  {
    ...
    if (tid == 0)
      ori_dscal_compute(EDGE_LEN,alpha,(double *)x,inc_x);
    else
      ori_dscal_compute(DIM_LEN,alpha,(double *)(x + EDGE_LEN + (tid - 1) * DIM_LEN), inc_x);
  }
  return;
}