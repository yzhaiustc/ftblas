...
void cblas_dsymm(const CBLAS_ORDER Layout, const CBLAS_SIDE side, const CBLAS_UPLO uplo, \
    ...
    const double beta, double *c, const FTBLAS_INT ldc) {
        ...
        bool case1 = (Layout==CblasColMajor) && ((side==CblasLeft) && (uplo==CblasLower));
...
#ifdef FT_ENABLED
        ftblas_dsymm_ft(m,n,k,alpha,(double *)a,lda,(double *)b,ldb,beta,(double *)c,ldc);
#else
        ftblas_dsymm_ori(m,n,k,alpha,(double *)a,lda,(double *)b,ldb,beta,(double *)c,ldc);
#endif
}