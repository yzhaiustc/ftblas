...
void cblas_dtbsv_compute(const CBLAS_ORDER layout, const CBLAS_UPLO Uplo, const CBLAS_TRANSPOSE Trans, const CBLAS_DIAG Diag,
...
{
    cblas_dtbsv_compute(layout, Uplo, Trans, Diag, n, k, (double *)a, lda, (double *)x, incx);
}
                         ...
                         FTBLAS_INT n, const FTBLAS_INT k, double *a, FTBLAS_INT lda, double *x, int incx)
{
    ...
    if (uplo == trans)
    {
        ftblas_dtbsv_upp(uplo, trans, unit, n, k, a, lda, x, incx);
    }
    else
    {
        ftblas_dtbsv_low(uplo, trans, unit, n, k, a, lda, x, incx);
    }
}